...
async function processCSV() {
  ...
  const GRANTED_STATUSES = new Set(['patented case', 'patent granted', 'patented file - (old case added for file tracking purposes)']);
  let rowCount = 0;
  let skipped = 0;
  ...
  await new Promise((resolve, reject) => {
    ...
    parser.on('readable', () => {
      ...
      while ((record = parser.read()) !== null) {
        rowCount++;
        if (rowCount % LOG_EVERY === 0) console.log(`  Processed ${rowCount.toLocaleString()} rows — ${examinerMap.size.toLocaleString()} examiners`);

        const rawName = record['examiner_full_name'];
        const formattedName = formatName(rawName);
        if (!formattedName) { skipped++; continue; }
        ...
        const isGranted = GRANTED_STATUSES.has(statusDesc) || (patentNumber !== '' && patentNumber !== '0');

        if (!examinerMap.has(key)) {
          examinerMap.set(key, { name: formattedName, art_unit_number: artUnit, total: 0, granted: 0, totalPendency: 0, pendencyCount: 0 });
        }

        const ex = examinerMap.get(key);
        ex.total++;
        if (isGranted) ex.granted++;
        if (artUnit && !ex.art_unit_number) ex.art_unit_number = artUnit;
  ...
  console.log(`\nFinished: ${rowCount.toLocaleString()} rows, ${examinerMap.size.toLocaleString()} examiners, ${skipped} skipped\n`);
  return examinerMap;
}

async function main() {
  console.log('=================================================');
  console.log(' PatentIQ — Ingest from application_data.csv');
  console.log('=================================================');
  ...
  for (const [, ex] of examinerMap) {
    ...
    rows.push({
      ...
      grant_rate_3yr: Math.round((ex.granted / ex.total) * 10000) / 100,
      pendency_months: ex.pendencyCount > 0 ? Math.round((ex.totalPendency / ex.pendencyCount) * 10) / 10 : null,
      total_applications: ex.total,
      updated_at: new Date().toISOString(),
    });
  }

  console.log(`Upserting ${rows.length.toLocaleString()} examiners...`);
  let processed = 0;
  let failed = 0;
  ...
  console.log('\n=================================================');
  console.log(` Upserted: ${processed} | Failed: ${failed} | Time: ${elapsed}s`);
  console.log('=================================================\n');
}
