...
export function Navbar() {
  ...
  // Display name: full name if available, otherwise first part of email
          ...
          {user ? (
            <div className="relative">
              <button onClick={() => setMenuOpen(o => !o)}
                ...
                <span className="text-sm font-medium text-slate-700 hidden sm:block max-w-32 truncate">{displayName}</span>
                ...
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-52 bg-white rounded-2xl border border-slate-100 shadow-xl z-20 overflow-hidden">
                    <div className="px-4 py-3 border-b border-slate-100">
                      ...
                      <p className="text-sm font-semibold text-slate-900 truncate">{fullName ?? user?.email}</p>
                    </div>
                    <div className="py-1">
            ...
            <>
              <Link href="/sign-in" className="text-sm font-medium text-slate-600 hover:text-slate-900 transition-colors px-4 py-2">Sign in</Link>
              <Link href="/sign-up" className="text-sm font-semibold text-white bg-blue-600 hover:bg-blue-700 active:scale-95 transition-all px-5 py-2 rounded-xl shadow-sm">Get started</Link>
            </>
          )}
        </div>
      </div>
